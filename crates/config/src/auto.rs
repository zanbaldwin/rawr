...
impl Configurator {
    /// Read raw data from a [`Figment`], apply auto-configuration, and store
    /// the result. The returned [`Configurator`] owns its data so the
    /// original figment can be consumed by `.join()` or `.merge()` afterward.
