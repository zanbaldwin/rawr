...
trait StorageBackendExt: StorageBackend {
    /// See [`StorageBackend::list`]
    async fn path_list(&self, prefix: Option<impl TryValidatePath>) -> Result<Vec<FileInfo>> {
    ...
    async fn path_exists(&self, path: impl TryValidatePath) -> Result<bool> {
        self.exists(&path.try_validate()?).await
    }

    /// See [`StorageBackend::read`]
    async fn path_read(&self, path: impl TryValidatePath) -> Result<Vec<u8>> {
        self.read(&path.try_validate()?).await
    }

    /// See [`StorageBackend::read_head`]
    async fn path_read_head(&self, path: impl TryValidatePath, bytes: usize) -> Result<Vec<u8>> {
        self.read_head(&path.try_validate()?, bytes).await
    }

    /// See [`StorageBackend::write`]
    async fn path_write(&self, path: impl TryValidatePath, data: &[u8]) -> Result<()> {
        self.write(&path.try_validate()?, data).await
    }

    /// See [`StorageBackend::delete`]
    async fn path_delete(&self, path: impl TryValidatePath) -> Result<()> {
        self.delete(&path.try_validate()?).await
    }

    /// See [`StorageBackend::rename`]
    async fn path_rename(&self, from: impl TryValidatePath, to: impl TryValidatePath) -> Result<()> {
        self.rename(&from.try_validate()?, &to.try_validate()?).await
    }

    /// See [`StorageBackend::stat`]
    async fn path_stat(&self, path: impl TryValidatePath) -> Result<FileInfo> {
        self.stat(&path.try_validate()?).await
    }

    /// See [`StorageBackend::reader`]
    async fn path_reader(&self, path: impl TryValidatePath) -> Result<BoxedReader> {
        self.reader(&path.try_validate()?).await
    }

    /// See [`StorageBackend::writer`].
    async fn path_writer(&self, path: impl TryValidatePath) -> Result<BoxedWriter> {
