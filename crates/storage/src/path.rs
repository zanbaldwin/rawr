...
/// // Valid paths
/// assert!(ValidPath::new("Fandom/work.html.bz2").is_ok());
/// assert!(ValidPath::new("a/b/c/file.html").is_ok());
/// assert!(ValidPath::new("a/../file.html").is_ok()); // (never leaves library root)
/// // Invalid paths
/// assert!(ValidPath::new("../etc/passwd").is_err());
/// assert!(ValidPath::new("a/../../b").is_err()); // (leaves library root)
/// assert!(ValidPath::new("a\0b").is_err());
/// // Paths get resolved
/// assert_eq!(
///     &ValidPath::new("wrong/../still-wrong/.././correct//./path.html/").unwrap(),
///     "correct/path.html"
/// );
/// ```
...
impl ValidPath {
    ...
    ///
    /// Can't implement `AsRef<Path>` for ValidPath (breaks the blanket
...
pub trait TryValidatePath {
    fn try_validate(self) -> Result<ValidPath>;
}
...
impl TryValidatePath for ValidPath {
    fn try_validate(self) -> Result<ValidPath> {
        Ok(self)
    }
}
...
// TODO: impl<T> TryFrom<T> for ValidPath where T: TryValidatePath {}
//       But it would require to not implement TryValidatePath for ValidPath,
...
impl PartialEq<str> for ValidPath {
...
mod tests {
    ...
    #[test]
    fn test_valid_paths() {
        assert_eq!(*ValidPath::new("Fandom/work.html.bz2").unwrap(), "Fandom/work.html.bz2");
        assert_eq!(*ValidPath::new("a/b/c/file.html").unwrap(), "a/b/c/file.html");
        assert_eq!(*ValidPath::new("simple.html").unwrap(), "simple.html");
    }

    #[test]
    fn test_path_normalization() {
        // Double slashes are normalized
        assert_eq!(*ValidPath::new("a//b//c").unwrap(), "a/b/c");
    ...
    fn test_reverse_attempts() {
        ...
        assert_eq!(*ValidPath::new("a/b/..").unwrap(), "a");
    }
    ...
    fn test_trailing_slashes() {
        ...
        assert_eq!(*ValidPath::new("Fandom1/").unwrap(), "Fandom1");
        assert_eq!(*ValidPath::new("a/b/c/").unwrap(), "a/b/c");
        assert_eq!(*ValidPath::new("file.html/").unwrap(), "file.html");
