...
pub(crate) struct ExportContext<'a> {
    ...
    pub(crate) output: Arc<Box<dyn Output>>,
...
impl<'a> ExportContext<'a> {
    pub(crate) async fn try_from_app(ctx: &'a AppContext) -> Result<Self> {
        ...
        Ok(Self {
            ...
            styles: build_style_config(&ctx.config.library.styles)?,
            path_generator,
...
fn build_style_config(styles: &[String]) -> Result<rawr_render::StyleConfig> {
    let mut config = StyleConfig::new();
    for entry in styles {
        config = if let Some(name) = entry.strip_prefix("builtin:") {
            config.with_builtin(name)?
        } else {
            config.with_file(entry)?
        };
    }
    Ok(config)
}
