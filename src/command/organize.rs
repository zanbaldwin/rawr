        ...
        while let Some(event) = stream.next().await {
            match event {
                ...
                Ok(OrganizeEvent::Organized(action)) => {
                    let line: Line = format_action_line(&action);
...
fn format_action_line<'a>(action: &'a Action) -> Line<'a> {
    let mut line = Line::empty();
    match action {
        Action::Renamed { from, to } => {
            line = line.with_fallback(format!("{from} -> {to}"));
            let old_tokens = tokenize_path(from);
            let new_tokens = tokenize_path(to);
            line.push(("· ", &PALETTE.muted).into());
            for change in TextDiff::from_slices(&old_tokens, &new_tokens).iter_all_changes() {
                ...
                line.push(Piece::fixed(change.value().to_string(), style));
            }
        },
        Action::AlreadyCorrect(p) => {
            line.push(("= ", &PALETTE.muted).into());
            line.push((p.as_str(), &PALETTE.muted).into());
        },
        Action::CleanedUp(p) => {
            line.push(("× ", &PALETTE.danger).into());
            line.push((p.as_str(), &PALETTE.danger).into());
        },
    };
    line
}
